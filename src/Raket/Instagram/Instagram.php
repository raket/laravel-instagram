...
class Instagram {
	 ...
	 * @return mixed
	 */
	public function getTagMedia($name, $args = false) {
		return $this->_makeCall('tags/' . $name . '/media/recent', $this->_accesstoken, $args);
	}
	...
	private function _makeCall($function, $auth = false, $params = null, $method = 'GET') {
		...
		if ('POST' === $method) {
			$response = $client->post($apiCall)->addPostFields($params)->send();
		} else if ('DELETE' === $method) {
			$response = $client->delete($apiCall)->send();
		}
		else {
			$response = $client->get($apiCall)->send();
		}
